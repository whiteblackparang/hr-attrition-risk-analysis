...
IBM HR Analytics 데이터를 활용해 조직 내 퇴사 리스크를 분석, 부서 및 직무 단위의 위험 요인을 도출하여 실무에서 활용 가능한 리스크 리포트를 구축한 End-to-End 데이터 분석 프로젝트.

Python(EDA·통계 검정), SQL(집계·윈도우 함수), Excel(자동화 리포트)을 활용한 End-to-End 분석 파이프라인을 구현.
...
| 핵심 결과 | Sales·HR 부서 위험군, Sales Representative 퇴사율 39.8% |
...
1. **야근과 만족도는 무관하지만, 야근과 퇴사는 강하게 연관**됨 (χ²=87.56, p<0.0001) — 설문 지표만으로는 리스크를 포착할 수 없음을 시사
...
![Overtime vs Attrition](outputs/charts/overtime_attrition_rate.png)
![Overtime vs Satisfaction](outputs/charts/boxplot_overtime_satisfaction.png)
![Department Attrition vs Income](outputs/charts/dept_attrition_vs_income.png)
![Job Role Risk Ranking](outputs/charts/jobrole_risk_ranking.png)
...
├── data/
│   ├── raw/                  # 원본 데이터
│   └── processed/            # 정제 데이터 (hr_cleaned.csv)
├── notebooks/                # 01_eda ~ 04_visualize
├── sql/
│   ├── hr_analysis.db
│   └── queries/               # 01_basic ~ 04_jobrole_ranking
├── excel/
│   └── HR_Risk_Report.xlsx
├── outputs/charts/           # 시각화 PNG
└── README.md
```
...
### 1. Python
`notebooks/01_eda_overtime_satisfaction.ipynb` 실행 → 통계 검정 결과 확인
`notebooks/02_clean_and_export.ipynb` 실행 → `data/processed/hr_cleaned.csv` 생성

### 2. SQLite
`notebooks/03_load_to_sqlite.ipynb` 실행 → `sql/hr_analysis.db` 생성
SQLite에서 `sql/queries/*.sql` 실행 → 부서별/직무별 리스크 통계 확인

### 3. Visualization
`notebooks/04_visualize.ipynb` 실행 → `outputs/charts/` 에 시각화 저장

### 4. Excel Dashboard
`excel/HR_Risk_Report.xlsx` 열어 대시보드 확인

---

## 개선 방향

- Power BI/Tableau 기반 부서별 Drill-down 대시보드 구축
- 로지스틱 회귀 등 예측 모델을 통한 개별 직원 단위 퇴사 확률 스코어링
- 다년도 데이터 확보 시 시계열 기반 리텐션 모니터링으로 확장
- 급여 변화를 가정한 정책 효과 시뮬레이션(시나리오 분석) 추가